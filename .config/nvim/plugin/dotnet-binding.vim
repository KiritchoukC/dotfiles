...
" Add interface
nnoremap <leader>zI _ww"zyiwA,<space>I<esc>"zp
" standard try catch
nnoremap <leader>zts cctry<CR>{<CR>}<CR>catch(Exception<space>ex)<CR>{<CR>ThrowException(ex,<space>methodName:<space>DebugHelper.GetMethodName(this));<CR>throw;<CR>}<ESC>
" event try catch
nnoremap <leader>zte cctry<CR>{<CR>}<CR>catch(Exception<space>ex)<CR>{<CR>HandleEventException(ex,<space>DebugHelper.GetMethodName(this),<space>e);<CR>}<ESC>
" copy outer method
